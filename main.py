...
# 针对 Windows WebView2 的全局安全设置
# 必须在 webview 启动前设置。强制禁用证书错误警告、混合内容警告，并将 Casdoor IP 视为安全来源
if platform.system() == 'Windows':
    os.environ['WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS'] = (
        '--ignore-certificate-errors '
        '--allow-running-insecure-content '
        '--unsafely-treat-insecure-origin-as-secure=http://66.154.108.88:8000'
    )
...
def WebViewApp(ifDev=False, ifCef=False):
    ...
    # 启动窗口
    webview.start(debug=Config.devEnv, http_server=True, http_port=Config.devPort, gui=guiCEF)
